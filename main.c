...
//Barcha talabalarni ko`rish uchun
void get_students(){
    if (student_count > 0)
    {
...
//talaba haqidagi malumotlarni chiqarish
void get_student_detail(){
    int ID_1;
...
//Ismi bo`yicha talabalarni chiqarish;
void get_student_by_name(){
    char search_name[40];
    if (student_count != 0)
    {   
...
void add_score(){
    if (student_count != 0)
    {
        if (lesson_count != 0)
        {
            int choose_student; //talabani tanlab olish un o`zgaruvchi
            printf("ID | First Name | Last Name | Age | Phone        | Email                   | Parent's Name | Parent's Phone | Parent's Email\n");
            for (int i = 0; i < student_count; i++)
            {
                printf("%-3d", i + 1);
            ...
            {   
                int choose_lesson; //darsni tanlash uchun o`zgaruvchi
                printf(" ID | Fanlar\n--------------------\n");
